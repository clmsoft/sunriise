...
public class KMPMatch {
     ...
     * 
     * public int scan(InputStream is, int start, int max) { int j = 0; if
     * (data.length == 0) return -1;
     * 
     * for (int i = start; i < start + max; i++) { while (j > 0 && match[j] !=
     * data[i]) j = failure[j - 1];
     * 
     * if (match[j] == data[i]) j++;
     * 
