...
public class EmbeddedPassword {
    private static final Logger log = Logger.getLogger(EmbeddedPassword.class);
    private FileChannel fileChannel;

    public EmbeddedPassword(File file, boolean readOnly) throws FileNotFoundException {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 1) {
            ...
            Class<EmbeddedPassword> clz = EmbeddedPassword.class;
            System.out.println("Usage: java " + clz.getName() + " file.mny [newPassword]");
            System.exit(1);
        }
        log.info("> file=" + file.getAbsolutePath());
        EmbeddedPassword password = null;
        try {
            boolean readOnly = true;
            if (newPassword != null) {
                readOnly = false;
            }
            password = new EmbeddedPassword(file, readOnly);
            if (readOnly) {
